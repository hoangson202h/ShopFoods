...
<div class="col-md-10"
	...
	<div class="content-box-large">
		...
		<form action="${pageContext.request.contextPath}/admin/table/index" style="margin-bottom: 10px;">
			<input value="tất cả" type="submit"/>
		</form>
		<div style="float: left;">
			 <select onchange="tableorder();" id="year">
				<c:forEach var="i" begin="2019" end="2025">
					<option <c:if test="${i == year }">selected="selected"</c:if> value="${i}">${i}</option>
				</c:forEach>
			</select>
		</div>
		<div style="float: left;margin-left: 10px;margin-bottom: 20px;">
			 <select onchange="tableorder();" id="month">
				<c:forEach var="i" begin="1" end="12">
					<option <c:if test="${i == month }">selected="selected"</c:if> value="${i}">Tháng ${i}</option>
				</c:forEach>
			</select>
		</div>
		<div style="float: left;margin-left: 10px;margin-bottom: 20px;">
			 <select onchange="tableorder();" id="day">
				<c:forEach var="i" begin="1" end="31">
					<option <c:if test="${i == day }">selected="selected"</c:if> value="${i}">Ngày ${i}</option>
				</c:forEach>
			</select>
		</div>
		<script type="text/javascript">
			function tableorder() {
				...
				var month = $('#month').val();
				var year = $('#year').val();
				window.location='${pageContext.request.contextPath}/admin/table/update-check/'+day+'/'+month+'/'+year;
			}
		</script>
		<div class="row">
						...
						<c:forEach items="${listtable}" var="item">
							...
							<tr class="odd gradeX">
								...
								<td><select name="check_st"
									onchange="check_st(this,${item.id});">
										<option value="0"
											<c:if test="${item.check_tb==0}">selected="selected"</c:if>>Chưa đến</option>
										<option value="1"
											<c:if test="${item.check_tb==1}">selected="selected"</c:if>>Đã xong</option>
								</select></td>
								<td class="center text-center"> <a
									onclick="return confirm('Hủy bàn? bạn có đồng ý?')"
									href="${delUrl}" title="" class="btn btn-danger"> <span
										class="glyphicon glyphicon-trash"></span> Hủy
								</a></td>
							</tr>
						</c:forEach>
					</tbody>
					...
					function check_st(obj,id) {
						...
						for (var i = 0; i < options.length; i++) {
							if (options[i].selected) {
								window.location='${pageContext.request.contextPath}/admin/table/update/'+options[i].value+'/'+id ;
							}
						}
					}
				</script>
